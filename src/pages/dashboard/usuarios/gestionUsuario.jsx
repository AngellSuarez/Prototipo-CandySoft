...
import { cambiar_estado_usuario, eliminar_usuario, crear_usuario, editar_usuario, listar_usuarios_administrativos } from "../../../services/usuarios_service"
...
const GestionUsuarios = () => {
        ...
        if (resultado.isConfirmed) {
            try {
                ...
                MySwal.fire(swalConfig).then(() => {
                    setUsuarios(prev => {
                        const actualizados = respuestaApi?.message === 'Usuario y sus asociados eliminados permanentemente'
                            ? prev.filter(u => u.id !== usuario.id)
                            : prev.map(u => u.id === usuario.id ? { ...u, estado: 'Inactivo' } : u);

                        const totalFiltrados = actualizados.filter(u =>
    ...
    const openEditarModal = (usuario) => {
        ...
        setModoVerUsuario(false); // Set view mode to false for editing
        setEditarModalOpen(true); // Use the same modal state
    };

    const closeEditarModal = () => {
        setUsuarioEditando(null);
        setEditarModalOpen(false);
        setErroresEditar({}); // Clear errors on close
        setModoVerUsuario(false); // Reset modoVerUsuario on close
    };
    ...
    const openVerModal = (usuario) => {
        setUsuarioEditando(usuario);
        setModoVerUsuario(true); // Set view mode to true
        setEditarModalOpen(true); // Use the same modal state
    };
    ...
    useEffect(() => {
        const obtener_roles = async () => {
            try {
                ...
                // Filter the roles here
                const filteredRoles = data.filter(rol =>
                    rol.nombre != "Manicurista" && rol.nombre != "Cliente"
                );
                setRoles(filteredRoles); // Set the filtered roles
                console.log(filteredRoles); // Log the filtered data
            } catch (error) {
                                ...
                                <form onSubmit={handle_crear_usuario} className="form-crear-usuario">
                                    ...
                                    <div className="fila-formulario">
                                        <div className="campo relative">
                                            <label htmlFor="tipo_documento" className="subtitulo-editar-todos">Tipo de documento:</label>
                                            <select name="tipo_documento" id="tipo_documento" className={selectClass("rol")}
                                        ...
                                        <div className="campo relative">
                                            <label htmlFor="apellido" className="subtitulo-editar-todos">Número de documento:</label>
                                    ...
                                    <div className="fila-formulario">
                                        <div className="campo relative">
                                            <label htmlFor="nombre" className="subtitulo-editar-todos">Nombres:</label>
                                            <input
                                                type="text"
                                                name="nombre"
                                        ...
                                        <div className="campo relative">
                                            <label htmlFor="apellido" className="subtitulo-editar-todos">Apellidos:</label>
                                            <input
                                                type="text"
                                                name="apellido"
                                            ...
                                            {formData.apellido.length === 40 && <p className="error-texto"> Has alcanzado el máximo de 40 caracteres.</p>}
                                        </div>
                                    </div>

                                    <div className="fila-formulario">
                                        <div className="campo">
                                            <label htmlFor="correo" className="subtitulo-editar-todos">Correo:</label>
                                            <input
                                                type="email"
                                                name="correo"
                                        ...
                                        <div className="campo relative">
                                            <label htmlFor="username" className="subtitulo-editar-todos">Nombre de usuario:</label>
                                            <input
                                                type="text"
                                                name="username"
                                            ...
                                            {formData.username.length === 20 && <p className="error-texto"> Has alcanzado el máximo de 20 caracteres.</p>}
                                        </div>
                                    </div>

                                    <div className="fila-formulario">
                                        <div className="campo">
                                            <label htmlFor="rol_id" className="subtitulo-editar-todos">Rol:</label>
                                            <select
                                                name="rol_id"
                                    ...
                                    onSubmit={async (e) => {
                                        ...
                                        try {
                                            ...
                                            if (respuesta?.errores) {
                                                ...
                                                if (respuesta.errores.username)
                                                    erroresBackend.username = respuesta.errores.username[0];

                                                if (respuesta.errores.correo)
                                                    erroresBackend.correo = respuesta.errores.correo[0];

                                                if (respuesta.errores.numero_documento)
                                                    erroresBackend.numero_documento = respuesta.errores.numero_documento[0];

                                                if (respuesta.errores.password)
                                                    erroresBackend.password = respuesta.errores.password[0];

                                                if (respuesta.errores.nombre)
                                                    erroresBackend.nombre = respuesta.errores.nombre[0];

                                                if (respuesta.errores.apellido)
                                                    erroresBackend.apellido = respuesta.errores.apellido[0];

                                                if (respuesta.errores.tipo_documento)
                                                    erroresBackend.tipo_documento = respuesta.errores.tipo_documento[0];

                                                if (respuesta.errores.rol_id)
                                                    erroresBackend.rol_id = respuesta.errores.rol_id[0];
                                ...
                                >
                                    <div className="grid grid-cols-2 gap-3">
                                        <div className="fila-formulario">
                                            ...
                                            <div className="campo relative">
                                                <label htmlFor="numero_documento" className="subtitulo-editar-todos">Número de documento:</label>
                                                <input
                                                    type="text"
                                                ...
                                                />
                                                {erroresEditar.numero_documento && !modoVerUsuario && <p className="error-mensaje">{erroresEditar.numero_documento}</p>}
                                                {usuarioEditando.numero_documento?.length === 15 && <p className="error-mensaje">Has alcanzado el máximo de 15 caracteres.</p>}
                                            </div>
                                        </div>
                                        <div className="fila-formulario">
                                            <div className="campo">
                                                ...
                                                <input
                                                    ...
                                                    style={{ pointerEvents: modoVerUsuario ? "none" : "auto" }}
                                                />
                                                {erroresEditar.nombre && !modoVerUsuario && <p className="error-mensaje">{erroresEditar.nombre}</p>}
                                                {usuarioEditando.nombre.length === 40 && !modoVerUsuario && <p className="error-mensaje">Has alcanzado el máximo de 40 caracteres.</p>}
                                            </div>
                                            ...
                                            <div className="campo">
                                                ...
                                                <input
                                                    ...
                                                    style={{ pointerEvents: modoVerUsuario ? "none" : "auto" }}
                                                />
                                                {erroresEditar.apellido && !modoVerUsuario && <p className="error-mensaje">{erroresEditar.apellido}</p>}
                                                {usuarioEditando.apellido.length === 40 && !modoVerUsuario && <p className="error-mensaje">Has alcanzado el máximo de 40 caracteres.</p>}
                                            </div>
                                        </div>
                                        ...
                                        <div className="fila-formulario">
                                            <div className="campo">
                                                ...
                                                <input
                                                    ...
                                                    style={{ pointerEvents: modoVerUsuario ? "none" : "auto" }}
                                                />
                                                {erroresEditar.correo && !modoVerUsuario && <p className="error-mensaje">{erroresEditar.correo}</p>}
                                            </div>
                                            ...
                                            <div className="campo">
                                                ...
                                                <input
                                                    ...
                                                    style={{ pointerEvents: modoVerUsuario ? "none" : "auto" }}
                                                />
                                                {erroresEditar.username && !modoVerUsuario && <p className="error-mensaje">{erroresEditar.username}</p>}
                                                {usuarioEditando.username.length === 20 && !modoVerUsuario && <p className="error-mensaje">Has alcanzado el máximo de 20 caracteres.</p>}
                                            </div>
                                        </div>
