...
import { MdInventory, MdLogout, MdShoppingCartCheckout } from "react-icons/md";
...
const BaseCrud = () => {
    ...
    const [confiOpen, setConfiOpen] = useState(false);
    ...
    const apellidoUsuario = localStorage.getItem("apellido") || "Usuario";
        ...
        <div className={`base-container ${menuOpen ? "menu-expanded" : "menu-collapsed"} ${darkMode ? "dark-mode" : "light-mode"}`}>
            ...
            <aside className={`sidebar ${menuOpen ? "open" : "closed"} ${darkMode ? "dark-mode" : ""}`}>
                ...
                <div className={`profile-section ${menuOpen ? "visible" : "hidden"}`}>
                    <img
                        src="https://i.pinimg.com/474x/ec/85/5e/ec855eda68d0f657ffebb24c95ebf552.jpg"
                        alt="Profile"
                ...
                <nav>
                    <ul>
                        ...
                        <li className="submenu-parent" onClick={() => setConfiOpen(!confiOpen)}>
                            <div className="submenu-toggle nav-item">
                                <Settings className="icon" />
                                {menuOpen && "Configuración"}
                                {!menuOpen && <span className="tooltip" style={{ top: "22.5%", left: "70px" }}>Configuración</span>}
                                {menuOpen && <span className="arrow">{confiOpen ? <FiChevronDown /> : <FiChevronRight />}</span>}
                            </div>
                            <ul className={`submenu ${confiOpen && menuOpen ? "show" : ""}`}>
                                <li className="nav-item">
                        ...
                        <li className="submenu-parent" onClick={() => setComprasOpen(!comprasOpen)}>
                            <div className="submenu-toggle nav-item">
                                ...
                                {menuOpen && "Compras"}
                                {!menuOpen && <span className="tooltip" style={{ top: "30.5%", left: "70px" }}>Compras</span>}
                            ...
                            <ul className={`submenu ${comprasOpen && menuOpen ? "show" : ""}`}>
                                <li className="nav-item">
                                    <Link to="/administrador/dashboard/marcas">
                                        <Tag className="icon" /> Marcas
                        ...
                        <li className="submenu-parent" onClick={() => setServiciosOpen(!serviciosOpen)}>
                            <div className="submenu-toggle nav-item">
                                ...
                                {menuOpen && "Servicios"}
                                {!menuOpen && <span className="tooltip" style={{ top: "39.9%", left: "70px" }}>Servicios</span>}
                        ...
                        <li className="submenu-parent" onClick={() => setVentasOpen(!ventasOpen)}>
                            <div className="submenu-toggle nav-item">
                                ...
                                {menuOpen && "Ventas"}
                                {!menuOpen && <span className="tooltip" style={{ top: "48%", left: "70px" }}>Ventas</span>}
                        ...
                        <li className="nav-item">
                            <Link to="#" onClick={(e) => {
                                ...
                                {!menuOpen && <span className="tooltip" style={{ top: "66.5%", left: "70px" }}>Cerrar Sesión</span>}
                            </Link>
                        </li>
                    </ul>
                </nav>
            </aside>
            <main className={`base-content ${darkMode ? "dark-mode" : ""}`}>
                <Outlet />
            </main>
        </div>
    );
